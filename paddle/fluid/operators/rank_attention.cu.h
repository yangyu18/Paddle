...
__global__ void expand_input_by_rank_kernel(
    ...
    int output_col, const T* rank_offset, int rank_offset_row,
...
__global__ void expand_rank_attention_param_kernel(
    const T* input, int input_row, int input_col, const T* rank_offset,
  ...
  CUDA_KERNEL_LOOP(idx, output_param_row * output_param_col) {
    ...
    int lower = rank_offset[ins_idx * rank_offset_col];
    int faster = rank_offset[2 * k + 1 + rank_offset_col * ins_idx];
    int flag = rank_offset[2 * k + 2 + rank_offset_col * ins_idx];
    if (flag < 0) continue;
...
__global__ void merge_input_gradient_kernel(
  ...
  CUDA_KERNEL_LOOP(tid, input_grad_row * input_grad_col) {
    ...
    int rank_idx = ins_rank[input_row_idx];
    int idx_offset = rank_idx * 2 + 2;

    T tmp = 0;
    for (int ins = 0; ins < input_grad_row; ++ins) {
      int flag = rank_offset[ins * rank_offset_col + idx_offset];
      if (input_row_idx == flag) {
        tmp += expanded_grad[ins * expanded_grad_col + input_col * rank_idx +
                             input_col_idx];
      } else if (input_row_idx < flag) {
        break;
      }
    }
    input_grad[tid] = tmp;
...
template <typename T>
void merge_rank_attention_input_grad(cudaStream_t stream, T* expanded_grad,
                                     int expanded_grad_row,
  ...
  merge_input_gradient_kernel<<<GET_BLOCKS(input_grad_row * input_grad_col),
                                CUDA_NUM_THREADS, 0, stream>>>(
      expanded_grad, expanded_grad_row, expanded_grad_col, input_grad,
      input_grad_row, input_grad_col, rank_offset, rank_offset_row,
      rank_offset_col, ins_rank, input_col);
}
