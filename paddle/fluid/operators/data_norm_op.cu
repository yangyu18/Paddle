...
template <typename T>
__global__ void gp(const T *s, int len) {
  for (int i = 0; i < len; i++) {
    printf("%lf ", s[i]);
  }
  printf("\n");
...
template <typename T>
void p(const T *s, int len, const std::string &msg) {
  printf("%s\n", msg.c_str());
  gp<<<1, 1>>>(s, len);
...
class DataNormKernel<platform::CUDADeviceContext, T>
 ...
 public:
  void Compute(const framework::ExecutionContext &ctx) const override {
    ...
    // p(batch_size_in, C, "batch_size");
    // p(batch_sum_in, C, "batch_sum");
    // p(batch_square_sum_in, C, "batch_square_sum");
    ...
    // p(mean_out_data, C, "mean_out");
    // p(scale_out_data, C, "scale_out");
...
class DataNormGradKernel<platform::CUDADeviceContext, T>
 ...
 public:
  void Compute(const framework::ExecutionContext &ctx) const override {
    ...
    // cudaStreamSynchronize(stream);
    // p(d_y->data<T>(), N*C, "d_y");
    // p(d_x->data<T>(), N*C, "d_x");
    ...
    kernel_data_norm_bp_stat<<<GET_BLOCKS(C), PADDLE_CUDA_NUM_THREADS, 0,
                               stream>>>(N, C, x->data<T>(), means->data<T>(),
                                         ...
                                         d_batch_square_sum);
    // cudaStreamSynchronize(stream);
    // p(d_batch_size, C, "d_batch_size");
    // p(d_batch_sum, C, "d_batch_sum");
    // p(d_batch_square_sum, C, "d_batch_square_sum");

    // synchonize among all GPU cards
    // sync mode now for paddlebox
    ...
    PADDLE_ENFORCE_CUDA_SUCCESS(platform::dynload::ncclAllReduce(
        ...
        platform::ToNCCLDataType(x->type()), ncclSum, comm->comm(),
        comm_stream));
    cudaError_t e_sync = cudaStreamSynchronize(comm_stream);
    if (e_sync != 0) {
    ...
    kernel_update_param<<<GET_BLOCKS(C), PADDLE_CUDA_NUM_THREADS, 0, stream>>>(
        ...
        batch_sum_data, batch_square_sum_data, 0.9999999);
  }
};
}  // namespace operators
}  // namespace paddle
