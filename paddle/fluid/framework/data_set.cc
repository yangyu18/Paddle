...
void DatasetImpl<T>::DynamicAdjustChannelNum(int channel_num,
  ...
  // FIXME for wasq model
  VLOG(3) << "input_pv_channle block size is "
...
void MultiSlotDataset::ShufflePVInstance() { return; }

void MultiSlotDataset::Divide_Pv_Instance() {
  // divide pv instance, and merge to input_channel_
  VLOG(3) << "MultiSlotDataset::Divide_Pv_Instance begin";
  platform::Timer timeline;
  timeline.Start();

  VLOG(3) << "multi_pv_consume_.size() " << multi_pv_consume_.size();
  input_channel_->Open();
  for (size_t i = 0; i < multi_pv_consume_.size(); ++i) {
    std::vector<PvInstance> pv_data;
    multi_pv_consume_[i]->Close();
    multi_pv_consume_[i]->ReadAll(pv_data);
    for (size_t j = 0; j < pv_data.size(); ++j) {
      input_channel_->Write(std::move(pv_data[j].ads));
    }
    pv_data.clear();
  ...
  timeline.Pause();
  VLOG(3) << "MultiSlotDataset::Divide_Pv_Instance end, cost time="
          << timeline.ElapsedSec() << " seconds";
  VLOG(3) << "input_channel_ has " << input_channel_->Size() << " instances";
}

void MultiSlotDataset::Merge_Pv_Instance() {
  VLOG(3) << "MultiSlotDataset::Merge_Pv_Instance begin";
  platform::Timer timeline;
  timeline.Start();

  if (!input_channel_ || input_channel_->Size() == 0) {
    VLOG(3) << "MultiSlotDataset::SortBySearchId() end, no data to shuffle";
    return;
  }
  auto fleet_ptr = FleetWrapper::GetInstance();
  input_channel_->Close();
  ...
  timeline.Pause();
  VLOG(3) << "MultiSlotDataset::Merge_Pv_Instance end, cost time="
          << timeline.ElapsedSec() << " seconds";
  VLOG(3) << "input_pv_channel has " << input_pv_channel_->Size()
          << " pv instance";
}

void MultiSlotDataset::SortBySearchId() {
  VLOG(3) << "MultiSlotDataset::SortBySearchId begin";
  platform::Timer timeline;
  timeline.Start();
  ...
  timeline.Pause();
  VLOG(3) << "MultiSlotDataset::SortBySearchId() end, cost time="
          << timeline.ElapsedSec() << " seconds";
